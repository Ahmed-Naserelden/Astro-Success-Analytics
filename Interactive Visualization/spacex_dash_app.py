...
@app.callback(
    ...
    [
        Input(component_id='site-dropdown', component_property='value'),
        Input(component_id='pyload-slider', component_property='value')
    ]
)
def get_scatter_chart(entered_site, payload_slider):
    ...
    if entered_site == 'All Sites':
        ...
        scatter_df = scatter_df[spacex_df['Launch Site'] == entered_site]
        fig = px.scatter(
            scatter_df,
            x='Payload Mass (kg)',
            y='class',
            hover_data=['Booster Version'],
            color='Booster Version Category',
            title=f'Correlation between Payload and Success for all Sites for {entered_site} sit',
        )
        return fig 
# Run the app
